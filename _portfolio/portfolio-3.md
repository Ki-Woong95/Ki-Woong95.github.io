...
The main point of the current project is to capture the stylistic metrics of each author, and utilize in these features into models to properly classify the given texts.
...
These books are stored in the directory `gutenberg`. Using the script `gutenberg/scripts/gutenberg_scrapping.ipynb`, I created an augmented dataset which has about 33% of mismatched pairs.
...
The current project examined whether stylometric features such as average word length, average span length and frequency of punctuation could help model which classifies whether
